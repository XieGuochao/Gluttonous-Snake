...
def main(main_screen):
    ...
    main_thread.join()
    waiting_thread = threading.Thread(target = wait_for_exit)
    waiting_thread.start()
    
    screen.addstr("Game Over!\n")
    screen.addstr("Your result: {}\n".format(state["state"]))
    screen.addstr("Your score: {}\n".format(state["score"]))
    screen.addstr("Enter / Space to Exit\n")
    screen.refresh()
